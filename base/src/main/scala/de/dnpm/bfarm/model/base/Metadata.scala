...
(
  ...
  researchConsents: List[Metadata.ResearchConsent],
  tanC: Id[TransferTAN],
...
object Metadata
{
  ...
  object DiseaseType extends Enumeration
  {
    ...
    val Rare        = Value("rare")
    // Hereditary not relevant for us
  ...
  object RejectionJustification extends Enumeration
  {
    ...
    val ProbablyNotGeneticCause      = Value("probablyNoGeneticCause")
  ...
  object ResearchConsent
  {
    ...
    object NoScopeJustification extends Enumeration
    {
      val PatientUnable        = Value("patient unable to consent")
      val PatientRefuses       = Value("patient refuses to sign consent")
      val NotReturned          = Value("patient did not return consent documents")
      val OtherPatientReason   = Value("other patient-related reason")
      val TechnicalReason      = Value("consent information cannot be submitted by LE due to technical reason")
      val OrganizationalIssues = Value("consent is not implemented at LE due to organizational issues")
