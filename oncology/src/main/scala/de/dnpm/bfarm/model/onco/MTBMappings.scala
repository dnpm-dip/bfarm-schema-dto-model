...
trait MTBMappings extends Mappings[MTBPatientRecord,OncologySubmission]
{
  ...
  protected implicit def oncoDiagnosis: MTBPatientRecord => OncologyCase.Diagnosis = {
    ...
    record =>
      ...
      OncologyCase.Diagnosis(
        ...
        Option.when(otherDiagnoses.nonEmpty)(
  ...
  ): MTBSystemicTherapy => Option[OncologyCase.PriorTherapy] = {

    import RECIST._

    therapy =>
      Option.when(therapy.period.isDefined)(
  ...
  protected implicit val followUpMapping: MTBPatientRecord => Option[OncologyFollowUps] = {
   ...
   record =>
     if (
       record.followUps.exists(
         fu => fu.nonEmpty && fu.maxBy(_.date).patientStatus.collect { case PatientStatus(LostToFU) => true }.isEmpty
       )
     ){
       ...
       val metachronousTumors =
         record.diagnoses.filter(_.`type`.latestBy(_.date).value.code.enumValue == MTBDiagnosis.Type.Metachronous)
  ...
  implicit val useCaseSubmission: mvh.Submission[MTBPatientRecord] => OncologySubmission =
